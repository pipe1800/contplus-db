...
BEGIN
  ...
  -- ========================================================================
  -- Delete rows from [12] that conflict with existing [6] rows (all tables)
  ...
  DELETE FROM plantilla_h WHERE cia = 12 AND (id_pda) IN (SELECT id_pda FROM plantilla_h WHERE cia = 6);
  DELETE FROM plantilla_d WHERE cia = 12 AND (id_partida, item) IN (SELECT id_partida, item FROM plantilla_d WHERE cia = 6);
